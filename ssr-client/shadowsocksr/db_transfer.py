		...
		try:
			while True:
				...
				if db_instance.event.wait(get_config().UPDATE_TIME):
					logging.info("wait break")
					break
				if not ServerPool.get_instance().thread.is_alive():
					logging.info("alive break")
					break
		...
		if self.cfg["ssl_enable"] == 1:
			conn = cymysql.connect(host=self.cfg["host"], port=self.cfg["port"],
					user=self.cfg["user"], passwd=self.cfg["password"],
					               db=self.cfg["db"], charset='utf8',connect_timeout=self.cfg["timeout"],
